#####################################
#
# NOTE! This file will only do sequence only / single sequence imputation
#
########################################
...
####################################################
def brnn_impute(directory_to_saved_network, input_list, output_types, file_list=None, network_size=[[256,256],[1024,512]], directory_to_save_files=None, print_results=False, input_file_dir=None, save_file_ext='.spd3', input_file_ext='.spd3'):
  # input_list is the list of input files.
  ...
  ph_LSTM_internal_dropout_keep_prob = tf.placeholder("float")
...
#                                          y, ph_output_mask,ph_output_mask_encoded, n_classes, ph_keep_prob)
      network = brnn_network.brnn_network(network_size, output_types, true_label_ind, pred_label_ind,
                                            x, n_input, ph_seq_len, ph_seq_len_mask, ph_bool_len_mask, 
                                            y, ph_output_mask,ph_output_mask_encoded, n_classes, ph_keep_prob,
                                            LSTM_internal_dropout_keep_prob=ph_LSTM_internal_dropout_keep_prob,
      ...
      if print_results is True:      
        print "Saving the best outputs of the test set to files in "+directory_to_save_files
        ...
        misc.save_predictions_to_file(test_network_output, test_seq_names, test_lengths, save_dir=directory_to_save_files, file_ext=save_file_ext, header='%s' % ', '.join(map(str, output_types)))
...
## Here is argparse style inputs.
  brnn_impute(args.directory_to_saved_networks, args.input_file_list, args.output_types, print_results=True, directory_to_save_files=args.directory_to_save_outputs, save_file_ext=args.save_ext, input_file_dir=args.input_dir, input_file_ext=args.input_ext)
